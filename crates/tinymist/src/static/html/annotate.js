...
(() => {
  ...
  const onMouseDown = (ev) => {
    if (!canAnnotate() || ev.button !== 0 || inBox(ev)) return;
  ...
  const showStatus = (lines) => {
    ...
    el.onclick = null;
    el.title = "";
    // A generated document cannot be annotated, and that is true for as long
    // as the page is open. It goes above whatever the compile has to say,
    // which comes and goes.
    el.classList.toggle("tm-status-refused", !!generated);
    if (generated) {
      const said = document.createElement("div");
      said.className = "tm-status-refusal";
      said.textContent = "autogenerated file; annotations disabled";
      const why = document.createElement("div");
      why.className = "tm-status-because";
      why.textContent = generated;
  ...
  const loadDocument = () =>
    getJson("/api/html/doc")
      .then((res) => {
        ...
        renderId = res.render;
        // Said by the document itself, so it is known before anything can be
        // pointed at.
        if ((res.generated || null) !== generated) {
          generated = res.generated || null;
          applyMode();
  ...
  //
  // A generated document is written whole every time whatever generates it
  // runs, so an anchor written into it lasts until the next build and no
  // longer. Annotating is refused rather than offered and then lost: the
  // server would keep such an annotation as one about the document, which is
  // not what somebody pointing at a word is asking for.
  let generated = null;

  // Whether the reader may annotate: in annotating mode, and not looking at a
  // document that is written afresh by something else.
  const canAnnotate = () => annotating && !generated;
  ...
  const applyMode = () => {
    ...
    root.classList.toggle("tm-generated", !!generated);
    if (!annotating) {
      clearHover();
      closeBox(true);
    }
